(() => {
  ...
  const api = {
    ...
    startGame() { return this.request('/api/runs/start', {method:'POST',body:'{}'}); },
    score(score, runId) { return this.request('/api/scores', {method:'POST',body:JSON.stringify({score,runId})}); }
  };
  ...
  async function validateSession(){const saved=readSession();if(!saved?.token||!saved.user){clearSession();requestAuth();return false;}try{const result=await api.me();currentUser=result.user;localStorage.setItem(sessionKey,JSON.stringify({token:saved.token,user:result.user}));updatePlayerLabels();return true;}catch{clearSession();requestAuth();return false;}}
  ...
  const runner={
    ...
    addObstacle(){const make=(type,x)=>{const img=new Image();img.src=A+(type==='box'?'obstaculo-1-caja.png':'obstaculo-2-mochila-removebg-preview.png');const scale=this.viewportScale;const h=(type==='box'?87:96)*scale,w=(type==='box'?113:106)*scale;return{x,y:this.ground-h,w,h,img};};let count=1;if(this.speedMultiplier>=3.2)count=4;else if(this.speedMultiplier>=2.6)count=3;else if(this.speedMultiplier>=2)count=2;const firstType=Math.random()<.5?'box':'bag';const first=make(firstType,W+30);this.obstacles.push(first);let right=first.x+first.w;for(let i=1;i<count;i++){const type=i%2===1?(firstType==='box'?'bag':'box'):firstType;const obstacle=make(type,first.x+first.w*.5*i);this.obstacles.push(obstacle);right=Math.max(right,obstacle.x+obstacle.w);}this.lastGroupWidth=right-first.x;},
    ...
    nextObstacleDelay(){const maxWidth=115*this.viewportScale;const airtime=(2*this.jumpVelocity)/this.gravity;const reachable=this.speed*airtime;return Math.max(520*this.viewportScale,reachable+this.lastGroupWidth+260*this.viewportScale)/this.speed;},
    ...
    draw(){ctx.clearRect(0,0,W,H);if(this.groundImage?.complete&&this.groundImage.naturalWidth){const stripH=Math.min(86,H*.105),sourceH=92;for(let x=-this.groundOffset;x<W;x+=W)ctx.drawImage(this.groundImage,0,this.groundImage.naturalHeight-sourceH,this.groundImage.naturalWidth,sourceH,x,this.ground,W,stripH);}this.sunflowers.forEach((s)=>{if(s.img.complete&&s.img.naturalWidth)ctx.drawImage(s.img,s.x,s.y,s.w,s.h);});this.obstacles.forEach((o)=>{if(o.img.complete&&o.img.naturalWidth)ctx.drawImage(o.img,o.x,o.y,o.w,o.h);});},
    async end(){this.running=false;cancelAnimationFrame(this.raf);this.restoreRunningFrame();playerElement.classList.add('is-hidden');const current=Math.floor(this.score);$('final-score').textContent=current;$('high-score').textContent=formatScore(Math.max(this.high,current));$('game-over-player').textContent=currentUser?`@${currentUser.username}`:'runner';$('score-result').textContent='Guardando tu mejor marca…';$('game-over').classList.remove('is-hidden');try{const result=await api.score(current,this.runId);this.high=result.user.score;$('high-score').textContent=formatScore(this.high);$('score-result').textContent=result.updated?'Nueva marca global registrada.':'Tu marca global no cambió.';loadLeaderboard();}catch(error){if(error.status===401){clearSession();$('game-over').classList.add('is-hidden');showScreen('menu');requestAuth();return;}$('score-result').textContent=error.message;}}
  };
  function updateOrientationLabel(){const active=document.body.classList.contains('landscape-mode');$('orientation-button').setAttribute('aria-pressed',String(active));$('orientation-button').title=active?'Volver a vista vertical':'Cambiar vista horizontal';$('device-status').textContent=active?'Vista horizontal adaptada activada':'';}
  function toggleLandscape(){document.body.classList.toggle('landscape-mode');localStorage.setItem('beliel-landscape-mode',document.body.classList.contains('landscape-mode')?'1':'0');updateOrientationLabel();}
  function updateFullscreenLabel(){const active=Boolean(document.fullscreenElement);$('fullscreen-button').setAttribute('aria-label',active?'Salir de pantalla completa':'Activar pantalla completa');$('fullscreen-button').title=active?'Salir de pantalla completa':'Activar pantalla completa';}
  async function toggleFullscreen(){const status=$('device-status');try{if(document.fullscreenElement){await document.exitFullscreen();}else if(document.documentElement.requestFullscreen){await document.documentElement.requestFullscreen();}else throw new Error('unsupported');status.textContent=document.fullscreenElement?'Pantalla completa activada.':'Pantalla completa desactivada.';}catch{status.textContent='La pantalla completa no está disponible en este navegador.';}updateFullscreenLabel();}
  $('orientation-button').addEventListener('click',toggleLandscape);$('fullscreen-button').addEventListener('click',toggleFullscreen);document.addEventListener('fullscreenchange',updateFullscreenLabel);$('start-button').addEventListener('click',()=>currentUser?runner.start():requestAuth());$('restart-button').addEventListener('click',()=>runner.start());$('menu-button').addEventListener('click',()=>{runner.stop();showScreen('menu');loadLeaderboard();});canvas.addEventListener('pointerdown',()=>runner.jump());document.addEventListener('keydown',(event)=>{if(!screens.game.classList.contains('is-hidden')&&(event.key===' '||event.key==='ArrowUp')){event.preventDefault();runner.jump();}});window.addEventListener('resize',()=>{if(!screens.game.classList.contains('is-hidden')){runner.resize();runner.syncPlayerElement();}});if(localStorage.getItem('beliel-landscape-mode')==='1')document.body.classList.add('landscape-mode');updateOrientationLabel();updateFullscreenLabel();
  (async()=>{const saved=readSession();if(saved?.token&&saved.user){currentUser=saved.user;updatePlayerLabels();if(!await validateSession())return;}else requestAuth();await loadLeaderboard();})();
})();
