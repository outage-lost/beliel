(() => {
  ...
  const dialogues = [
    ...
    'He estado intentando ser mejor y conseguir algo con lo que yo pueda tener éxito. Lo hacía por muchas personas, y también por ti y por Belén.',
  ...
  const runner = {running:false,raf:0,last:0,score:0,high:Number(localStorage.getItem('milena-high-score')||0),speed:330,ground:0,player:{x:0,y:0,w:65,h:112,vy:0,onGround:true},obstacles:[],spawnIn:1.3,sheet:null,frame:0,frameTime:0,
    resize(){dpr=Math.min(2,window.devicePixelRatio||1);W=canvas.clientWidth;H=canvas.clientHeight;canvas.width=W*dpr;canvas.height=H*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);this.ground=H*.78;this.player.x=Math.max(35,W*.12);if(this.player.onGround)this.player.y=this.ground-this.player.h;},
    async start(){this.stop();showScreen('game');this.resize();this.score=0;this.speed=Math.max(300,W*.29);this.obstacles=[];this.spawnIn=1.2;this.player.vy=0;this.player.onGround=true;this.player.y=this.ground-this.player.h;$('score').textContent='00000';$('high-score').textContent=String(this.high).padStart(5,'0');$('game-over').classList.add('is-hidden');this.running=true;this.last=performance.now();if(!this.sheet)this.loadSheet();this.raf=requestAnimationFrame(t=>this.loop(t));},
    stop(){this.running=false;cancelAnimationFrame(this.raf);},
    async loadSheet(){try{const data=await fetch(A+'avatar-animado-mil-20260721-041007.piskel').then(r=>r.json());const uri=JSON.parse(data.piskel.layers[0]).chunks[0].base64PNG;this.sheet=new Image();this.sheet.src=uri;}catch(e){this.sheet=null;}},
    ...
    loop(now){if(!this.running)return;const dt=Math.min(.032,(now-this.last)/1000);this.last=now;this.score+=dt*10;this.speed+=dt*2.5;this.spawnIn-=dt;if(this.spawnIn<=0)this.addObstacle();const p=this.player;p.vy+=1850*dt;p.y+=p.vy*dt;if(p.y>=this.ground-p.h){p.y=this.ground-p.h;p.vy=0;p.onGround=true;}this.frameTime+=dt;if(this.frameTime>.11){this.frame=(this.frame+1)%11;this.frameTime=0;}this.obstacles.forEach(o=>o.x-=this.speed*dt);this.obstacles=this.obstacles.filter(o=>o.x+o.w>-30);const hitbox={x:p.x+13,y:p.y+8,w:p.w-25,h:p.h-12};for(const o of this.obstacles){if(hitbox.x<o.x+o.w-8&&hitbox.x+hitbox.w>o.x+8&&hitbox.y<o.y+o.h&&hitbox.y+hitbox.h>o.y+8){this.end();return;}}this.draw();this.raf=requestAnimationFrame(t=>this.loop(t));},
    draw(){ctx.clearRect(0,0,W,H);ctx.fillStyle='rgba(48,77,60,.75)';ctx.fillRect(0,this.ground,W,5);for(const o of this.obstacles){if(o.img.complete)ctx.drawImage(o.img,o.x,o.y,o.w,o.h);}if(this.sheet&&this.sheet.complete){ctx.drawImage(this.sheet,this.frame*311,0,311,363,this.player.x,this.player.y,this.player.w,this.player.h);}else{ctx.fillStyle='#304d3c';ctx.beginPath();ctx.arc(this.player.x+32,this.player.y+20,18,0,Math.PI*2);ctx.fill();ctx.fillRect(this.player.x+12,this.player.y+38,40,70);} $('score').textContent=String(Math.floor(this.score)).padStart(5,'0');},
