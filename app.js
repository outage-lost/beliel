(() => {
  ...
  async function lockLandscape(){const status=$('device-status');try{if(!screen.orientation?.lock)throw new Error('unsupported');await screen.orientation.lock('landscape');status.textContent='Modo horizontal activado.';}catch{status.textContent='Este navegador no permite bloquear el giro. Gira el teléfono manualmente para jugar en horizontal.';}}
  function updateFullscreenLabel(){$('fullscreen-button').textContent=document.fullscreenElement?'SALIR DE PANTALLA COMPLETA':'PANTALLA COMPLETA';}
  async function toggleFullscreen(){const status=$('device-status');try{if(document.fullscreenElement){await document.exitFullscreen();}else if(document.documentElement.requestFullscreen){await document.documentElement.requestFullscreen();}else throw new Error('unsupported');status.textContent=document.fullscreenElement?'Pantalla completa activada.':'Pantalla completa desactivada.';}catch{status.textContent='Este navegador no permite la pantalla completa desde aquí.';}updateFullscreenLabel();}
  $('orientation-button').addEventListener('click',lockLandscape);$('fullscreen-button').addEventListener('click',toggleFullscreen);document.addEventListener('fullscreenchange',updateFullscreenLabel);$('start-button').addEventListener('click',()=>currentUser?runner.start():requestAuth());$('restart-button').addEventListener('click',()=>runner.start());$('menu-button').addEventListener('click',()=>{runner.stop();showScreen('menu');loadLeaderboard();});canvas.addEventListener('pointerdown',()=>runner.jump());document.addEventListener('keydown',(event)=>{if(!screens.game.classList.contains('is-hidden')&&(event.key===' '||event.key==='ArrowUp')){event.preventDefault();runner.jump();}});window.addEventListener('resize',()=>{if(!screens.game.classList.contains('is-hidden')){runner.resize();runner.syncPlayerElement();}});updateFullscreenLabel();
